...
@PackageScope class PendingFormula implements Resolvable {
    ...
    protected String expandNames(String withNames) {
        withNames.replaceAll(/\#\{(.+?)\}/) { List<String> found ->
            ...
            if (!found) {
                throw new IllegalArgumentException("Named cell '${found[1]}' cannot be found! Please, take a note that the name was normalized to ${PoiCellDefinition.fixName(found[1])} due the Excel constraints.")
            }
